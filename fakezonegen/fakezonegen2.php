...
$block_domains = in_array('domains', $argv);
...
try {
	...
	// Теперь запросим ресурсы, которые нужно блокировать вместе со всеми поддоменами (domain-mask)
	$in_extra = $block_domains ? ', \'domain\'' : '';
	$statement = $db->prepare("SELECT domain_text FROM roskom_domain WHERE domain_content_id IN (SELECT content_id FROM roskom_content  WHERE content_block_type IN ('domain-mask'$in_extra))");
	$statement->execute();
	while($row = $statement->fetch(PDO::FETCH_ASSOC)) {
		$domain = str_replace('*.', '', $row['domain_text']);
		$domains[$domain] = $IDN->encode($domain);
	}
	$statement->closeCursor();
